...
/**
 * CollabAI brand logo — inline SVG mark + wordmark.
 *
 * Replaces the traced-bitmap SVGs (159KB embedded-PNG files whose letter spacing
 * rendered inconsistently). Inline SVG inherits the page font stack and themes
 * itself via currentColor / var(--brand), so it stays crisp at every size and in
 * every theme (dark / light / ocean).
 *
 * Usage:
 *   <app-logo />                // default size
...
@Component({
  ...
  styles: `
    ...
    .lg .brand-mark { height: 34px; }
    .lg .brand-name { font-size: 24px; }
...
export class LogoComponent {
  ...
  /** Render only the cat-C mark (collapsed sidebar, buttons). */
